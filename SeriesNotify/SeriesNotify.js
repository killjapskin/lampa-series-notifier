...
var arrayValue = safeClone(value[i], depth + 1);
...
try {
var objectValue = safeClone(value[key], depth + 1);

if (objectValue !== null) {
objectResult[key] = objectValue;
}
} catch (error) {}
}
...
typeof subscription.movie_id !== 'undefined'
) {
return 'id:' + String(subscription.movie_id);
}

return 'title:' + normalizeText(subscription.title);
}
...
score += Math.floor(
Number(subscription.updated_at || 0) / 1000000000
);
...
function mergeSubscriptionData(preferred, secondary) {
preferred = preferred || {};
secondary = secondary || {};

var merged = safeClone(preferred) || {};
...
merged[key] = safeClone(secondary[key]);
}
}

merged.new_files = mergeNewFiles(
preferred.new_files,
secondary.new_files
);

merged.created_at = Math.min(
Number(preferred.created_at || Date.now()),
Number(secondary.created_at || Date.now())
);

merged.updated_at = Math.max(
Number(preferred.updated_at || 0),
Number(secondary.updated_at || 0)
);

merged.id = getSubscriptionId(merged);
...
var movieKey = getMovieKey(current);
...
if (!grouped[movieKey]) {
grouped[movieKey] = current;
order.push(movieKey);
continue;
}

var existing = grouped[movieKey];

if (
getSubscriptionScore(current) >
getSubscriptionScore(existing)
) {
grouped[movieKey] = mergeSubscriptionData(
current,
existing
);
} else {
...
function findBestSubscriptionByMovie(movieId, title) {
var subscriptions = getSubscriptions();
var best = null;
var bestScore = -1;
...
sameMovie =
normalizeText(subscription.title) ===
normalizeText(title);
}
...
var score = getSubscriptionScore(subscription);
...
season:
typeof item.season !== 'undefined'
? item.season
: null,

episode:
typeof item.episode !== 'undefined'
? item.episode
: null
});
}
...
if (
hasSeriesValue(selectedFile.season) ||
hasSeriesValue(selectedFile.episode)
) {
return true;
}
...
if (
hasSeriesValue(item.season) ||
hasSeriesValue(item.episode) ||
hasSeriesPattern(item.path) ||
hasSeriesPattern(item.title)
) {
return true;
}
}
...
var files =
subscription &&
Array.isArray(subscription.new_files)
...
var pending = getPendingNewFiles(subscription);

subscription.has_update = pending.length > 0;

subscription.new_episode = pending.length
? (
pending[pending.length - 1].episode ||
...
for (var i = 0; i < subscriptions.length; i++) {
refreshUpdateState(subscriptions[i]);
...
return 'Обновления (' + getNotificationCount() + ')';
}
...
if (Lampa.Torrent.start.seriesNotifyWrapped) {
return true;
}

var originalStart = Lampa.Torrent.start;

function wrappedStart(torrentObject, movie) {
pendingTorrentObject = null;
pendingTorrentMovie = null;

if (torrentObject) {
pendingTorrentObject = safeClone(torrentObject);
}

var activeMovie = movie || getActiveMovie();

if (activeMovie) {
pendingTorrentMovie = safeClone(activeMovie);
}
...
wrappedStart.seriesNotifyWrapped = true;
wrappedStart.seriesNotifyOriginal = originalStart;

Lampa.Torrent.start = wrappedStart;
...
return safeClone(subscription.movie_object);
}

return createFallbackMovie(subscription);
}

function applyMovieToActiveActivity(movieObject) {
if (
!Lampa.Activity ||
typeof Lampa.Activity.active !== 'function'
) {
...
active.movie = movieObject;
active.card = movieObject;
active.item = movieObject;

if (
active.object &&
typeof active.object === 'object'
) {
active.object.movie = movieObject;
active.object.card = movieObject;
}
}
...
subscription = findBestSubscriptionByMovie(
cardData.id,
...
var movieObject = getSavedMovieObject(subscription);
var torrentObject = safeClone(
subscription.torrent_object
);

applyMovieToActiveActivity(movieObject);

pendingTorrentMovie = safeClone(movieObject);
pendingTorrentObject = safeClone(torrentObject);
...
var movieObject = pendingTorrentMovie
? safeClone(pendingTorrentMovie)
: safeClone(params.movie || {});

var movie = movieObject || {};

var torrentObject = pendingTorrentObject
? safeClone(pendingTorrentObject)
: null;

pendingTorrentMovie = null;
pendingTorrentObject = null;

var currentFiles = normalizeTorrentFiles(
event.items ||
params.files ||
[]
);
...
torrent_link:
getTorrentLink(torrentObject),

torrent_tracker:
getTorrentTracker(torrentObject),
...
var existing = findBestSubscriptionByMovie(
subscription.movie_id,
subscription.title
);

if (existing) {
subscription = mergeSubscriptionData(
subscription,
existing
);
}
...
refreshUpdateState(subscription);

var subscriptions = getSubscriptions();
var movieKey = getMovieKey(subscription);
var cleaned = [];

for (var i = 0; i < subscriptions.length; i++) {
if (
getMovieKey(subscriptions[i]) !==
movieKey
) {
cleaned.push(subscriptions[i]);
}
}

cleaned.push(subscription);
...
function playerMatchesFile(playerData, newFile) {
if (
!playerData ||
!newFile
) {
...
var playerKey = normalizeFileKey(playerData);
...
if (!Array.isArray(subscription.new_files)) {
subscription.new_files = [];
}

for (
var j = 0;
j < subscription.new_files.length;
j++
) {
var newFile = subscription.new_files[j];

if (newFile.loaded_in_player) {
continue;
}
...
newFile.loaded_in_player = true;
newFile.loaded_at = Date.now();
changed = true;
break;
}
}
...
function getSubscriptionSortTime(subscription) {
var time = Number(
...
var files = Array.isArray(subscription.new_files)
? subscription.new_files
: [];
...
subscriptions.sort(function (first, second) {
refreshUpdateState(first);
refreshUpdateState(second);

var firstUpdate = first.has_update ? 1 : 0;
var secondUpdate = second.has_update ? 1 : 0;
...
var timeDifference =
getSubscriptionSortTime(second) -
getSubscriptionSortTime(first);
...
var sameId =
cardData.series_notify_id &&
subscription.id === cardData.series_notify_id;

var sameMovie =
cardData.id !== null &&
typeof cardData.id !== 'undefined' &&
subscription.movie_id !== null &&
typeof subscription.movie_id !== 'undefined' &&
String(subscription.movie_id) ===
String(cardData.id);
...
cardData.series_notify_deleted = true;
...
function confirmRemoveSubscription(cardData, card) {
if (
cardData &&
cardData.series_notify_deleted
) {
...
setTimeout(function () {
var removed = removeSubscription(cardData);
...
for (var i = 0; i < subscriptions.length; i++) {
var subscription = refreshUpdateState(
subscriptions[i]
);
...
series_notify_has_update: subscription.has_update,
series_notify_pending:
getPendingNewFiles(subscription).length,
series_notify_sort_time:
getSubscriptionSortTime(subscription)
});
}
...
var component = new Lampa.InteractionCategory(
object
);
...
rendered &&
rendered.length &&
element.series_notify_has_update
) {
rendered.addClass(
'series-notify-card-update'
);
...
actions.prepend(createTopButton());
}
...
typeof Lampa.Player.listener.follow ===
'function'
) {
